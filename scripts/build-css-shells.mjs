...
const SECTIONS = [
  ...
  { start: 688,  shell: 'core',    label: 'Button vocabulary (.btn family)' },
  { start: 1257, shell: 'core',    label: 'Footer' },
  { start: 1354, shell: 'article', label: 'Citation drawer (.cite, .cite-body)' },
  ...
  { start: 1383, shell: 'core',    label: 'Global focus-visible + skip-link + sr-only + nav-toggle + mobile-menu (every-page chrome)' },
  ...
  { start: 1451, shell: 'core',    label: 'Nav refinements + share widget (was inside CITATION DRAWER section)' },
  { start: 1472, shell: 'article', label: 'Listen / audio player' },
  { start: 2001, shell: 'core',    label: 'Breadcrumbs' },
  { start: 2027, shell: 'core',    label: 'Homepage utility classes' },
  { start: 2079, shell: 'core',    label: 'Legal pages' },
  { start: 2105, shell: 'core',    label: 'Homepage primary tool CTA (restaurant audit)' },
  { start: 2395, shell: 'tool',    label: 'TOOL SHELL — shared primitives for /tools/* pages' },
  { start: 2493, shell: 'tool',    label: 'Tools-landing cluster layout' },
  { start: 2784, shell: 'tool',    label: 'Per-tool "Keep going" knit-in' },
  { start: 2895, shell: 'article', label: 'Glossary scannability' },
  { start: 3050, shell: 'article', label: '"Recently added" strip on /learn/' },
  { start: 3125, shell: 'core',    label: 'Library nav-mega regroup (nav is everywhere)' },
  { start: 3444, shell: 'article', label: 'Learn hub + Start here pages' },
  { start: 3547, shell: 'core',    label: '/system/ colophon page (small, kept in core)' },
  { start: 3624, shell: 'core',    label: 'Search modal (Pagefind-backed; on every page)' },
  { start: 3799, shell: 'article', label: 'Research notes (/learn/research/)' },
  { start: 3816, shell: 'article', label: 'Library topics (/learn/topics/)' },
  { start: 3998, shell: 'article', label: 'Glossary term pages (/glossary/<slug>/)' },
  ...
  { start: 4112, shell: 'tool',    label: 'Tool deep-links (.tool-deep-* — bottom of every /tools/* page)' },
  { start: 4196, shell: 'article', label: 'Resume article — services-aside-cta + see-also + research notes' },
  { start: 4593, shell: 'article', label: 'Research-note CTA inside cite drawer' },
  { start: 4614, shell: 'article', label: 'Research drawer (inline preview)' },
  { start: 4754, shell: 'article', label: 'Glossary term → research note cross-link' },
  { start: 4799, shell: 'article', label: '"Recently added" rail (under glossary hero)' },
  { start: 4857, shell: 'article', label: 'Glossary index "▶ 90s explainer" chip' },
  { start: 4878, shell: 'article', label: 'Glossary explainer (90-second narrated diagram)' },
  { start: 5144, shell: 'article', label: 'Inline glossary popover' },
  { start: 5211, shell: 'article', label: 'Print view for glossary section landing pages' },
  { start: 5268, shell: 'core',    label: 'Cloudflare Turnstile widget (reservation min-height)' },
  { start: 5280, shell: 'core',    label: 'Workshop save banner (multi-context, kept in core)' },
  { start: 5332, shell: 'tool',    label: 'Tool states (loading / error / empty)' },
  { start: 5377, shell: 'article', label: 'Editorial callouts' },
  { start: 5461, shell: 'tool',    label: 'Learn-back (in tool result region)' },
  { start: 5479, shell: 'article', label: 'Post-end Workshop CTA' },
  { start: 5495, shell: 'core',    label: 'Workshop rationale' },
  { start: 5511, shell: 'core',    label: 'The Window (/window/ — kept in core to avoid a 4th shell)' },
  { start: 5628, shell: 'core',    label: 'Window composer (Phase-2 redesign)' },
  { start: 5661, shell: 'core',    label: 'ADMIN /admin/window/ (kept in core; admin is auth-gated noindex)' },
  { start: 5748, shell: 'tool',    label: '.edu-result — interpretation card under tool output' },
  { start: 5787, shell: 'tool',    label: 'Statistical disclosure components' },
  { start: 5830, shell: 'tool',    label: 'Tool-internal type minimums' },
  ...
  { start: 5987, shell: 'core',    label: 'Newsletter capture (in global footer; on every page)' },
  { start: 6008, shell: 'article', label: 'Inline graphics — globalized from gold articles' },
  { start: 6069, shell: 'core',    label: 'Touch-device hover hygiene' },
  { start: 6108, shell: 'core',    label: 'Hero mobile reorder (homepage)' },
  { start: 6131, shell: 'article', label: 'KnitRail — "what’s next" component for articles' },
  { start: 6206, shell: 'core',    label: 'Hero count chips (homepage)' },
  { start: 6237, shell: 'core',    label: 'Trust strip (homepage)' },
  { start: 6263, shell: 'core',    label: 'Compare cards (homepage)' },
];
