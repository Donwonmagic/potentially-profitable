...
const SECTIONS = [
  ...
  { start: 2272, shell: 'core',    label: 'Breadcrumbs' },
  { start: 2298, shell: 'core',    label: 'Homepage utility classes' },
  { start: 2350, shell: 'core',    label: 'Legal pages' },
  { start: 2376, shell: 'core',    label: 'Homepage primary tool CTA (restaurant audit)' },
  { start: 2666, shell: 'tool',    label: 'TOOL SHELL — shared primitives for /tools/* pages' },
  { start: 2764, shell: 'tool',    label: 'Tools-landing cluster layout' },
  { start: 3055, shell: 'tool',    label: 'Per-tool "Keep going" knit-in' },
  { start: 3166, shell: 'article', label: 'Glossary scannability' },
  { start: 3321, shell: 'article', label: '"Recently added" strip on /learn/' },
  { start: 3396, shell: 'core',    label: 'Library nav-mega regroup (nav is everywhere)' },
  { start: 3715, shell: 'article', label: 'Learn hub + Start here pages' },
  { start: 3818, shell: 'core',    label: '/system/ colophon page (small, kept in core)' },
  { start: 3895, shell: 'core',    label: 'Search modal (Pagefind-backed; on every page)' },
  { start: 4070, shell: 'article', label: 'Research notes (/learn/research/)' },
  { start: 4087, shell: 'article', label: 'Library topics (/learn/topics/)' },
  { start: 4269, shell: 'article', label: 'Glossary term pages (/glossary/<slug>/)' },
  ...
  { start: 4383, shell: 'tool',    label: 'Tool deep-links (.tool-deep-* — bottom of every /tools/* page)' },
  { start: 4467, shell: 'article', label: 'Resume article — services-aside-cta + see-also + research notes' },
  { start: 4864, shell: 'article', label: 'Research-note CTA inside cite drawer' },
  { start: 4885, shell: 'article', label: 'Research drawer (inline preview)' },
  { start: 5025, shell: 'article', label: 'Glossary term → research note cross-link' },
  { start: 5070, shell: 'article', label: '"Recently added" rail (under glossary hero)' },
  { start: 5128, shell: 'article', label: 'Glossary index "▶ 90s explainer" chip' },
  { start: 5149, shell: 'article', label: 'Glossary explainer (90-second narrated diagram)' },
  { start: 5415, shell: 'article', label: 'Inline glossary popover' },
  { start: 5482, shell: 'article', label: 'Print view for glossary section landing pages' },
  { start: 5539, shell: 'core',    label: 'Cloudflare Turnstile widget (reservation min-height)' },
  { start: 5551, shell: 'core',    label: 'Workshop save banner (multi-context, kept in core)' },
  { start: 5603, shell: 'tool',    label: 'Tool states (loading / error / empty)' },
  { start: 5648, shell: 'article', label: 'Editorial callouts' },
  { start: 5732, shell: 'tool',    label: 'Learn-back (in tool result region)' },
  { start: 5750, shell: 'article', label: 'Post-end Workshop CTA' },
  { start: 5766, shell: 'core',    label: 'Workshop rationale' },
  { start: 5782, shell: 'core',    label: 'The Window (/window/ — kept in core to avoid a 4th shell)' },
  { start: 5899, shell: 'core',    label: 'Window composer (Phase-2 redesign)' },
  { start: 5932, shell: 'core',    label: 'ADMIN /admin/window/ (kept in core; admin is auth-gated noindex)' },
  { start: 6019, shell: 'tool',    label: '.edu-result — interpretation card under tool output' },
  { start: 6058, shell: 'tool',    label: 'Statistical disclosure components' },
  { start: 6101, shell: 'tool',    label: 'Tool-internal type minimums' },
  ...
  { start: 6258, shell: 'core',    label: 'Newsletter capture (in global footer; on every page)' },
  { start: 6279, shell: 'article', label: 'Inline graphics — globalized from gold articles' },
  { start: 6340, shell: 'core',    label: 'Touch-device hover hygiene' },
  { start: 6379, shell: 'core',    label: 'Hero mobile reorder (homepage)' },
  { start: 6402, shell: 'article', label: 'KnitRail — "what’s next" component for articles' },
  { start: 6477, shell: 'core',    label: 'Hero count chips (homepage)' },
  { start: 6508, shell: 'core',    label: 'Trust strip (homepage)' },
  { start: 6534, shell: 'core',    label: 'Compare cards (homepage)' },
  ...
  // /blog/, /learn/, /glossary/.
  { start: 6618, shell: 'article', label: 'Article viz components (.viz-* family)' },
  ...
  { start: 6957, shell: 'core',    label: 'Window Phase 2/3.6/4/5+ additions (sash/sidelight, attach, now, pulses, admin)' },
  ...
  { start: 7194, shell: 'core',    label: 'Phase 1 — design tokens + MuntinUI component shells (.mtn-*)' },
  ...
  { start: 7406, shell: 'core',    label: 'Phase 2 — hub tier badge + tool-tier-filter strip' },
  ...
  { start: 7459, shell: 'core',    label: 'Phase 5 — dark-mode token override (prefers-color-scheme + [data-theme])' },
  ...
  { start: 7519, shell: 'core',    label: 'Phase 7 — dark-mode toggle button (.theme-toggle)' },
  ...
  { start: 7563, shell: 'core',    label: 'Phase 4 — next-tool recommendation card (.mtn-next-tool)' },
  ...
  { start: 7603, shell: 'core',    label: 'Phase G — About portrait frame (real photo overlay)' },
];
