...
const SECTIONS = [
  ...
  { start: 2199, shell: 'core',    label: 'Breadcrumbs' },
  { start: 2225, shell: 'core',    label: 'Homepage utility classes' },
  { start: 2277, shell: 'core',    label: 'Legal pages' },
  { start: 2303, shell: 'core',    label: 'Homepage primary tool CTA (restaurant audit)' },
  { start: 2593, shell: 'tool',    label: 'TOOL SHELL — shared primitives for /tools/* pages' },
  { start: 2691, shell: 'tool',    label: 'Tools-landing cluster layout' },
  { start: 2982, shell: 'tool',    label: 'Per-tool "Keep going" knit-in' },
  { start: 3093, shell: 'article', label: 'Glossary scannability' },
  { start: 3248, shell: 'article', label: '"Recently added" strip on /learn/' },
  { start: 3323, shell: 'core',    label: 'Library nav-mega regroup (nav is everywhere)' },
  { start: 3642, shell: 'article', label: 'Learn hub + Start here pages' },
  { start: 3745, shell: 'core',    label: '/system/ colophon page (small, kept in core)' },
  { start: 3822, shell: 'core',    label: 'Search modal (Pagefind-backed; on every page)' },
  { start: 3997, shell: 'article', label: 'Research notes (/learn/research/)' },
  { start: 4014, shell: 'article', label: 'Library topics (/learn/topics/)' },
  { start: 4196, shell: 'article', label: 'Glossary term pages (/glossary/<slug>/)' },
  ...
  { start: 4310, shell: 'tool',    label: 'Tool deep-links (.tool-deep-* — bottom of every /tools/* page)' },
  { start: 4394, shell: 'article', label: 'Resume article — services-aside-cta + see-also + research notes' },
  { start: 4791, shell: 'article', label: 'Research-note CTA inside cite drawer' },
  { start: 4812, shell: 'article', label: 'Research drawer (inline preview)' },
  { start: 4952, shell: 'article', label: 'Glossary term → research note cross-link' },
  { start: 4997, shell: 'article', label: '"Recently added" rail (under glossary hero)' },
  { start: 5055, shell: 'article', label: 'Glossary index "▶ 90s explainer" chip' },
  { start: 5076, shell: 'article', label: 'Glossary explainer (90-second narrated diagram)' },
  { start: 5342, shell: 'article', label: 'Inline glossary popover' },
  { start: 5409, shell: 'article', label: 'Print view for glossary section landing pages' },
  { start: 5466, shell: 'core',    label: 'Cloudflare Turnstile widget (reservation min-height)' },
  { start: 5478, shell: 'core',    label: 'Workshop save banner (multi-context, kept in core)' },
  { start: 5530, shell: 'tool',    label: 'Tool states (loading / error / empty)' },
  { start: 5575, shell: 'article', label: 'Editorial callouts' },
  { start: 5659, shell: 'tool',    label: 'Learn-back (in tool result region)' },
  { start: 5677, shell: 'article', label: 'Post-end Workshop CTA' },
  { start: 5693, shell: 'core',    label: 'Workshop rationale' },
  { start: 5709, shell: 'core',    label: 'The Window (/window/ — kept in core to avoid a 4th shell)' },
  { start: 5826, shell: 'core',    label: 'Window composer (Phase-2 redesign)' },
  { start: 5859, shell: 'core',    label: 'ADMIN /admin/window/ (kept in core; admin is auth-gated noindex)' },
  { start: 5946, shell: 'tool',    label: '.edu-result — interpretation card under tool output' },
  { start: 5985, shell: 'tool',    label: 'Statistical disclosure components' },
  { start: 6028, shell: 'tool',    label: 'Tool-internal type minimums' },
  ...
  { start: 6185, shell: 'core',    label: 'Newsletter capture (in global footer; on every page)' },
  { start: 6206, shell: 'article', label: 'Inline graphics — globalized from gold articles' },
  { start: 6267, shell: 'core',    label: 'Touch-device hover hygiene' },
  { start: 6306, shell: 'core',    label: 'Hero mobile reorder (homepage)' },
  { start: 6329, shell: 'article', label: 'KnitRail — "what’s next" component for articles' },
  { start: 6404, shell: 'core',    label: 'Hero count chips (homepage)' },
  { start: 6435, shell: 'core',    label: 'Trust strip (homepage)' },
  { start: 6461, shell: 'core',    label: 'Compare cards (homepage)' },
  ...
  // /blog/, /learn/, /glossary/.
  { start: 6545, shell: 'article', label: 'Article viz components (.viz-* family)' },
  ...
  { start: 6884, shell: 'core',    label: 'Window Phase 2/3.6/4/5+ additions (sash/sidelight, attach, now, pulses, admin)' },
  ...
  { start: 7121, shell: 'core',    label: 'Phase 1 — design tokens + MuntinUI component shells (.mtn-*)' },
  ...
  { start: 7333, shell: 'core',    label: 'Phase 2 — hub tier badge + tool-tier-filter strip' },
  ...
  { start: 7386, shell: 'core',    label: 'Phase 5 — dark-mode token override (prefers-color-scheme + [data-theme])' },
  ...
  { start: 7446, shell: 'core',    label: 'Phase 7 — dark-mode toggle button (.theme-toggle)' },
  ...
  { start: 7490, shell: 'core',    label: 'Phase 4 — next-tool recommendation card (.mtn-next-tool)' },
  ...
  { start: 7530, shell: 'core',    label: 'Phase G — About portrait frame (real photo overlay)' },
];
