...
const SECTIONS = [
  ...
  { start: 2050, shell: 'core',    label: 'Breadcrumbs' },
  { start: 2076, shell: 'core',    label: 'Homepage utility classes' },
  { start: 2128, shell: 'core',    label: 'Legal pages' },
  { start: 2154, shell: 'core',    label: 'Homepage primary tool CTA (restaurant audit)' },
  { start: 2444, shell: 'tool',    label: 'TOOL SHELL — shared primitives for /tools/* pages' },
  { start: 2542, shell: 'tool',    label: 'Tools-landing cluster layout' },
  { start: 2833, shell: 'tool',    label: 'Per-tool "Keep going" knit-in' },
  { start: 2944, shell: 'article', label: 'Glossary scannability' },
  { start: 3099, shell: 'article', label: '"Recently added" strip on /learn/' },
  { start: 3174, shell: 'core',    label: 'Library nav-mega regroup (nav is everywhere)' },
  { start: 3493, shell: 'article', label: 'Learn hub + Start here pages' },
  { start: 3596, shell: 'core',    label: '/system/ colophon page (small, kept in core)' },
  { start: 3673, shell: 'core',    label: 'Search modal (Pagefind-backed; on every page)' },
  { start: 3848, shell: 'article', label: 'Research notes (/learn/research/)' },
  { start: 3865, shell: 'article', label: 'Library topics (/learn/topics/)' },
  { start: 4047, shell: 'article', label: 'Glossary term pages (/glossary/<slug>/)' },
  ...
  { start: 4161, shell: 'tool',    label: 'Tool deep-links (.tool-deep-* — bottom of every /tools/* page)' },
  { start: 4245, shell: 'article', label: 'Resume article — services-aside-cta + see-also + research notes' },
  { start: 4642, shell: 'article', label: 'Research-note CTA inside cite drawer' },
  { start: 4663, shell: 'article', label: 'Research drawer (inline preview)' },
  { start: 4803, shell: 'article', label: 'Glossary term → research note cross-link' },
  { start: 4848, shell: 'article', label: '"Recently added" rail (under glossary hero)' },
  { start: 4906, shell: 'article', label: 'Glossary index "▶ 90s explainer" chip' },
  { start: 4927, shell: 'article', label: 'Glossary explainer (90-second narrated diagram)' },
  { start: 5193, shell: 'article', label: 'Inline glossary popover' },
  { start: 5260, shell: 'article', label: 'Print view for glossary section landing pages' },
  { start: 5317, shell: 'core',    label: 'Cloudflare Turnstile widget (reservation min-height)' },
  { start: 5329, shell: 'core',    label: 'Workshop save banner (multi-context, kept in core)' },
  { start: 5381, shell: 'tool',    label: 'Tool states (loading / error / empty)' },
  { start: 5426, shell: 'article', label: 'Editorial callouts' },
  { start: 5510, shell: 'tool',    label: 'Learn-back (in tool result region)' },
  { start: 5528, shell: 'article', label: 'Post-end Workshop CTA' },
  { start: 5544, shell: 'core',    label: 'Workshop rationale' },
  { start: 5560, shell: 'core',    label: 'The Window (/window/ — kept in core to avoid a 4th shell)' },
  { start: 5677, shell: 'core',    label: 'Window composer (Phase-2 redesign)' },
  { start: 5710, shell: 'core',    label: 'ADMIN /admin/window/ (kept in core; admin is auth-gated noindex)' },
  { start: 5797, shell: 'tool',    label: '.edu-result — interpretation card under tool output' },
  { start: 5836, shell: 'tool',    label: 'Statistical disclosure components' },
  { start: 5879, shell: 'tool',    label: 'Tool-internal type minimums' },
  ...
  { start: 6036, shell: 'core',    label: 'Newsletter capture (in global footer; on every page)' },
  { start: 6057, shell: 'article', label: 'Inline graphics — globalized from gold articles' },
  { start: 6118, shell: 'core',    label: 'Touch-device hover hygiene' },
  { start: 6157, shell: 'core',    label: 'Hero mobile reorder (homepage)' },
  { start: 6180, shell: 'article', label: 'KnitRail — "what’s next" component for articles' },
  { start: 6255, shell: 'core',    label: 'Hero count chips (homepage)' },
  { start: 6286, shell: 'core',    label: 'Trust strip (homepage)' },
  { start: 6312, shell: 'core',    label: 'Compare cards (homepage)' },
  ...
  // /blog/, /learn/, /glossary/.
  { start: 6396, shell: 'article', label: 'Article viz components (.viz-* family)' },
  ...
  { start: 6735, shell: 'core',    label: 'Window Phase 2/3.6/4/5+ additions (sash/sidelight, attach, now, pulses, admin)' },
  ...
  { start: 6972, shell: 'core',    label: 'Phase 1 — design tokens + MuntinUI component shells (.mtn-*)' },
  ...
  { start: 7184, shell: 'core',    label: 'Phase 2 — hub tier badge + tool-tier-filter strip' },
  ...
  { start: 7237, shell: 'core',    label: 'Phase 5 — dark-mode token override (prefers-color-scheme + [data-theme])' },
  ...
  { start: 7297, shell: 'core',    label: 'Phase 7 — dark-mode toggle button (.theme-toggle)' },
  ...
  { start: 7341, shell: 'core',    label: 'Phase 4 — next-tool recommendation card (.mtn-next-tool)' },
  ...
  { start: 7381, shell: 'core',    label: 'Phase G — About portrait frame (real photo overlay)' },
];
