...
- /cost-index/yields.json — edible-portion yields for 118 common restaurant ingredients (the usable share of purchased weight after standard trim and waste). Joins on slug to the price index; lets you turn a wholesale purchase price into a true cost per usable pound. Reference data, not a price.
- /cost-index/ingredient-codes.json — the identity crosswalk (also .csv): every ingredient slug mapped to the federal/agency codes that stand for it — US Census HS, USDA NASS, USDA ERS, NOAA Fisheries, GHCN weather regions, the NOAA CPC ONI index. Two columns say how far each identity can be trusted: "granularity" (line finer than a commodity / commodity / group coarser / proxy — a growing region standing in for the ingredient, not an identity of it) and "binding", where code_shared_by_slugs marks a code the source CANNOT split, so any per-slug figure drawn from it is really the combined figure for every ingredient named in "shared_with" (HS 080550 is lemon AND lime; NASS CATTLE stands for five beef cuts). An identity map, never a measurement: no price, no volume, no share, no forecast. CC BY 4.0.
...
- /open/ — seventeen in-browser data explorers over the raw public series behind the index, each stating its own source and license (US-gov data CC0; Muntin derivations CC-BY): the seasonality reader (/open/seasonality/ — why seasonal price curves exist and where timing the calendar actually pays), the import shelf (/open/imports/), export ledger (/open/exports/), import origins (/open/origins/), US domestic supply (/open/nass-domestic/), wild seafood landings (/open/landings/), per-capita availability (/open/availability/), the meat farm→wholesale→retail chain (/open/meat-chain/), the food dollar (/open/food-dollar/), corn & soybean condition (/open/crop-condition/), the energy backdrop (/open/energy/), origin-region weather (/open/weather/), the ENSO/ONI record (/open/oni/), the fused ingredient state record (/open/record/), the FDA food-recall record (/open/recalls/ — dated recalls tagged to ingredients as co-occurrence, never joined to a price, never a cause), the county restaurant-wage backdrop (/open/labor/ — a descriptive labor lane, never in the food index), and the food-service demand backdrop (/open/demand/ — observed MARTS sales, latest month provisional). Descriptive of the tracked record only — never a forecast.
- /cost-index/menu-pricing/study/ — a citable field report (ScholarlyArticle, CC-BY 4.0) grounding a per-ingredient menu-pricing method in 36 peer-reviewed sources (economics + food science, with DOIs). Its machine-readable evidence table (/cost-index/menu-pricing/study/study.json + .csv) maps each of the paper's claims to the sources that ground them, with each source's finding, the myth it corrects, and its confidence; the per-ingredient numbers behind the figures are the companion /cost-index/menu-pricing.json (+ .csv). Its "Ask this paper" layer owns seven questions, each answered with its number and its type welded in so the answer stays true quoted short (FAQPage): how to price a menu against inflation (37 lock / 19 cushion / 7 float / 37 withhold of 100); whether to reprint the menu on a spike (median ~77 days, IQR 50–105); how to price on yield not the invoice (trim tax = 1 ÷ yield; citrus category 2.16×, mushroom 1.14×, individual rows vary — lime 2.86×); whether a cheaper swap actually saves (432 episodes, 94% co-moved — check pair by pair); whether a cheaper month is worth buying (54 of 100 clear the seasonal noise gate); the real year-one restaurant failure rate (≈26%, not 90%); and whether ingredient cost matters to a thin-margin plate (farm value share ≈15.9¢, waste ROI ≈7:1). Descriptive of the tracked record and the literature — never a forecast.
...
El Muntin Cost Index publica semanalmente precios mayoristas de referencia para ingredientes comunes de restaurante, a partir de fuentes públicas de EE. UU. (USDA AMS/LMR, BLS, FRED, EIA, NOAA). Solo niveles, rangos y amplitudes medidos — nunca una dirección ni un pronóstico de precio (el backtest de 25 años muestra que las llamadas de dirección están a nivel de azar, 50.5% vs una base de 50.2%, así que no publicamos ninguna). Datos abiertos que puedes usar y citar libremente, bajo la licencia declarada de cada archivo (recálculo de datos del gobierno de EE. UU. CC0 / dominio público; compilaciones de Muntin CC BY 4.0 — citando a "Muntin Digital"). Superficies legibles por máquina:
...
- /cost-index/yields.json — rendimiento de porción comestible para 118 ingredientes comunes de restaurante (la parte utilizable del peso comprado tras la limpieza y el desperdicio). Se une por slug al índice de precios; convierte un precio mayorista de compra en un costo real por libra utilizable. Datos de referencia, no un precio.
- /cost-index/ingredient-codes.json — el mapa de identidad (también .csv): cada slug de ingrediente asociado a los códigos federales que lo representan — HS del Censo de EE.UU., USDA NASS, USDA ERS, NOAA Fisheries, regiones meteorológicas GHCN y el índice ONI de NOAA CPC. Dos columnas indican cuánto se puede confiar en cada identidad: "granularity" (line más fino que una mercancía / commodity / group más grueso / proxy — una región de cultivo que sustituye al ingrediente, no una identidad de él) y "binding", donde code_shared_by_slugs marca un código que la fuente NO puede separar, así que cualquier cifra por slug obtenida de él es en realidad la cifra combinada de todos los ingredientes nombrados en "shared_with" (HS 080550 es limón Y lima; NASS CATTLE representa cinco cortes de res). Un mapa de identidad, nunca una medición: sin precio, sin volumen, sin participación, sin pronóstico. CC BY 4.0.
...
- /open/ — dieciséis exploradores de datos en el navegador sobre las series públicas crudas detrás del índice, cada uno con su propia fuente y licencia (datos del gobierno de EE. UU. CC0; derivaciones de Muntin CC-BY): la estantería de importaciones (/open/imports/), el libro de exportaciones (/open/exports/), los orígenes de importación (/open/origins/), el suministro nacional de EE. UU. (/open/nass-domestic/), los desembarques de marisco silvestre (/open/landings/), la disponibilidad per cápita (/open/availability/), la cadena de la carne del campo a la venta minorista (/open/meat-chain/), el dólar de comida (/open/food-dollar/), la condición del maíz y la soja (/open/crop-condition/), el contexto energético (/open/energy/), el clima de las regiones de origen (/open/weather/), el registro ENSO/ONI (/open/oni/), el registro estatal de ingredientes (/open/record/), el registro de retiros de alimentos de la FDA (/open/recalls/ — retiros con fecha vinculados a ingredientes como co-ocurrencia, nunca unido a un precio, nunca una causa), el telón de fondo de salarios de restaurantes del condado (/open/labor/ — un carril laboral descriptivo, nunca en el índice de costos) y el telón de fondo de demanda del sector gastronómico (/open/demand/ — ventas observadas de MARTS, último mes provisional). Descriptivo del registro seguido — nunca un pronóstico.
- /cost-index/menu-pricing/study/ — un informe de campo citable (ScholarlyArticle, CC-BY 4.0) que fundamenta un método de precios de menú por ingrediente en 36 fuentes revisadas por pares (economía + ciencia de los alimentos, con DOI). Su tabla de evidencia legible por máquina (/cost-index/menu-pricing/study/study.json + .csv) mapea cada afirmación del informe a las fuentes que la respaldan, con el hallazgo de cada fuente, el mito que corrige y su confianza; los números por ingrediente detrás de las figuras son el complemento /cost-index/menu-pricing.json (+ .csv). Su capa "Pregúntale a este informe" posee siete preguntas, cada una respondida con su número y su tipo soldados para que la respuesta siga siendo cierta citada en corto (FAQPage): cómo fijar precios frente a la inflación (37 fijan / 19 amortiguan / 7 flotan / 37 se reservan de 100); si reimprimir el menú ante un pico (mediana ~77 días, IQR 50–105); cómo fijar precio sobre el rendimiento y no la factura (impuesto de merma = 1 ÷ rendimiento; categoría cítricos 2.16×, hongos 1.14×, las filas individuales varían — lima 2.86×); si un cambio más barato realmente ahorra (432 episodios, 94% co-movidos — comprobar par por par); si vale la pena comprar en el mes más barato (54 de 100 superan el filtro de dispersión); la tasa real de fracaso en el primer año (≈26%, no 90%); y si el costo del ingrediente importa para un plato de margen delgado (participación de valor de la finca ≈15.9¢, ROI de desperdicio ≈7:1). Descriptivo del registro seguido y de la literatura — nunca un pronóstico.
