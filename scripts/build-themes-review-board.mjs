...
function emitPage(locale) {
  ...
  const toolHref       = 'https://ledger.muntin.digital/';
...
<div class="container">
  ...
  <section class="md-rb-intro">
    ...
    <p>${escHtml(intro1)}</p>
    <p>${escHtml(intro2)}</p>
    <p><a href="${escHtml(toolHref)}" style="color:var(--teal);font-weight:600;border-bottom:1px dashed currentColor;text-decoration:none">${locale === 'es' ? 'Tus facturas, leídas y archivadas — Muntin Ledger →' : 'Your invoices, read and filed — Muntin Ledger →'}</a></p>
  </section>
