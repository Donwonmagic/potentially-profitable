...
// Clamp a meta-description to <= `max` rendered glyphs (SERP truncation
// ceiling). Operates on the PLAIN/decoded string — callers pass decoded
// text and pageHead esc()s it afterward, so length here equals the
// code-point length the meta-description gate measures. We trim to the
// last whole word that fits (leaving room for a single-glyph ellipsis),
// strip any dangling separator/punctuation, and never cut mid-word or
// mid-entity (there are no entities in the decoded input to split). No
// text is fabricated — only existing source is shortened.
function clampDescription(text, max = 155) {
  const s = String(text == null ? '' : text).replace(/\s+/g, ' ').trim();
  if ([...s].length <= max) return s;
  // Budget one glyph for the trailing ellipsis.
  const cut = [...s].slice(0, max - 1).join('');
  const lastSpace = cut.lastIndexOf(' ');
  let head = lastSpace > 0 ? cut.slice(0, lastSpace) : cut;
  // Drop trailing punctuation / separators so we don't emit "word —…"
  // or "word,…". Keep closing brackets/quotes off the tail too.
...
function pageHead(locale, { title, description, canonical, ogImage, jsonLd }) {
  // SERP hygiene: clamp the meta/og/twitter description to <=155 glyphs.
  // Centralised here so every renderer (terms, section hubs, topic pages,
  // topics hub) clears the meta-description-length gate without each
  // composing its own truncation. JSON-LD descriptions are built from the
  // full source separately and are intentionally NOT clamped.
  description = clampDescription(description, 155);
...
function renderTermPage(locale, term, allTerms) {
  ...
  // "Headword: definition." — pageHead clamps the whole thing to <=155
  // glyphs at a word boundary, so compose from the full source here and
  // let the central clamp trim the tail cleanly (no mid-word/entity cut).
  const desc = `${headPlain}: ${defPlain.trim()}`;
