...
(function(){
  ...
  // Unified footer treatment across all five Cards: ink ground, single
  // recessive cream-on-ink line at 60% opacity. Calm credit, not loud.
  // 48 px keep-out from each edge.
  function drawFooter(ctx, footerStrings){
    ...
    ctx.textBaseline = 'middle';
    ctx.textAlign = 'left';
    ctx.fillText(footerStrings.left, 48, H_PANE + H_FOOTER / 2);
    ctx.textAlign = 'right';
    ctx.fillText(footerStrings.right, W - 48, H_PANE + H_FOOTER / 2);
    ctx.restore();
  }
  ...
  // the Card paints with its proper display face. No-op in environments
  // without document.fonts (jsdom, older browsers) — the fallback stack
  // still renders intentionally.
  function awaitFonts(){
  ...
  var DEFAULT_STRINGS_EN = {
    ...
    footer: {
      left: 'Generated by Muntin · Brand Suite · muntin.digital',
      right: new Date().toISOString().slice(0, 10)
    }
  };
  ...
  var DEFAULT_STRINGS_ES = {
    ...
    footer: {
      left: 'Generado por Muntin · Suite de Marca · muntin.digital',
      right: new Date().toISOString().slice(0, 10)
    }
  };
