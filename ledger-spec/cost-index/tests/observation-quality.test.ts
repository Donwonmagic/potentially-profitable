/**
 * observation-quality.test.ts — VERBATIM port of the storefront suite.
 * The garbage-in defense: a poisoned obs must LOWER confidence, never silently
 * move a confident number. unit_mismatch is the one hard reject.
 */
import { test } from 'vitest';
import assert from 'node:assert/strict';
import * as Q from '../src/observation-quality.js';
...
test('mildly out of band is KEPT but down-weighted (a real shock is not silenced)', () => {
  const v = Q.validateObservation(
    { source: 'a', basis: 'wholesale', valueCents: 2400, date: '2026-05-01' },
    { bounds: { minCents: 1000, maxCents: 2000 } });
...
test('screen filters hard rejects and reports the lowest per-source weight', () => {
  const r = Q.screen([
    ...
    { source: 'x', basis: 'wholesale', valueCents: 2400, date: '2026-05-02' },
    { source: 'y', basis: 'wholesale', valueCents: 50000, date: '2026-05-01', unit: 'case' },
  ], { bounds: { minCents: 1000, maxCents: 2000 }, expectedUnit: 'lb' });
  assert.equal(r.kept.length, 2);
  assert.equal(r.rejected.length, 1);
  assert.equal(r.sourceWeight.x, 0.4);
});

test('INVARIANT: a poisoned obs cannot move the composite level (rejected, level unchanged)', () => {
  const clean: Q.QualityObs[] = [
  ...
  const cleanLevel = C.compositeLevel(clean as any)!;
  ...
  const safeLevel = C.compositeLevel(screened.kept as any)!;
  assert.equal(screened.rejected.length, 1);
  assert.equal(safeLevel.medianCents, cleanLevel.medianCents);
