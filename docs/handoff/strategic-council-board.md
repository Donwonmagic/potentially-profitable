...
- **Dispatch SHIPPED (`41da6f225`).** Assembled the gate-passing blog HTML from `data/pending-dispatch.json` via the canonical `new-article-skeleton.mjs` + a body renderer:
  ...
  - **`iterate-menu-pricing-experts` workflow — a 6-expert panel (controller, menu consultant, statistician, ag-economist, chef, editor) → judge → build → 3-lens adversarial verify, looping "until the product is perfect."** Founder: "continue in the cadence, iterate until perfect, grounded in expert insights." Each expert reads the piece cold + verifies vs the engine; content-safe picks get built + verified; only survivors applied.
    ...
    - **HELD from round 3 — verification incomplete (session limit), do NOT apply until re-verified:** (ii) **cohort-size distribution behind the 94%** (statistician) — verifiers flagged a "pack-size artifacts" mischaracterization to fix; needs the statistician agent's re-analysis before build. Re-run the panel/build+verify after the limit resets (Workflow scriptPath iterate-menu-pricing-experts, resumeFromRunId wf_76643307-207 replays cached agents). **(i) own-data ratchet and (iii) cooked-pound / second haircut are now SHIPPED (3b, 3c above) — only the cohort-distribution build remains held.**
