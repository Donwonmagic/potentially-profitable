...
**Standing queue is now CLEAR.** Next-thread candidates (from the queue further below): A — Muntin Plate emergent-insight catalog (design doc); B — vertical-generality build (product, separate repo); C — social pre-launch (needs founder IG decision); D — remaining website prune follow-ups (the 8 off-funnel tools as a scoped migration, `/method/` nav-link removal). Recommend confirming direction before the next build.
