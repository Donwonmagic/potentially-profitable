...
- **SHIPPED (later 2026-07-18, continued):** the data corpus is now **complete** — every roadmap Tier-1/1½ source landed + fused.
  ...
  - **Open-data catalog: 15 datasets** (10 CC0 gov / 5 CC-BY Muntin); every `data/*.jsonl` served with a `Link: rel=license` header.
...
- **Two workflows in flight:** `muntin-isr-improvement-loop` (w4rti0m0u — 5 surfaces → verified ranked plan; events-explorer re-dreamt separately with AEO/SEO + visual-depth, 8 findings incl. a timeline ribbon + window-overlap strip + caveat-baked FAQPage) and `muntin-dataset-explorers` (wdk0whgpn — 10 explorers; resume adds crop-condition + landings from cache).
