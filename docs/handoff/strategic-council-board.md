...
- **Phase 1 (heartbeat harvest) — IN PROGRESS.**
  ...
  - **Remaining:** dateModified for the OTHER 5 hubs (homepage EN/ES via hand-edit; blog/tools/glossary via their builders — exact JSON-LD targets in the Phase-1 scout); `/changelog/` JSON-LD `dateModified`=2026-05-02 is stale (prose says 2026-06-26). Weekly dispatch automation = **already DONE** (cron publishes+commits+emails; verify `COST_INDEX_BROADCAST_SECRET` is set).
