...
- **Dispatch SHIPPED (`41da6f225`).** Assembled the gate-passing blog HTML from `data/pending-dispatch.json` via the canonical `new-article-skeleton.mjs` + a body renderer:
  ...
  - **`iterate-menu-pricing-experts` workflow — a 6-expert panel (controller, menu consultant, statistician, ag-economist, chef, editor) → judge → build → 3-lens adversarial verify, looping "until the product is perfect."** Founder: "continue in the cadence, iterate until perfect, grounded in expert insights." Each expert reads the piece cold + verifies vs the engine; content-safe picks get built + verified; only survivors applied.
    ...
    - **QUEUED from the content panel (verified, not yet built) — additive polish, good next pass:** (a) **generalize the flagship** "buying season runs against the calendar" from 4 proteins to the **41** ingredients with both a cheapMonth + inSeasonMonths (needs the against-calendar directional count verified before writing); (b) **accessibility:** a scannable definition key for the coined postures (lock/cushion/float/withhold have no glossary surface); aria-hide or cut the pullquote that repeats the withhold-paragraph closer verbatim; break the ~200-word withhold paragraph into scannable pieces. (Lemon trim-tax "seam" ×2.22 vs juiceYield ×2.38 was CHECKED and left — ×2.22 is grounded in the card's yieldPct 45; juiceYield is a different metric, not a fabrication.)
