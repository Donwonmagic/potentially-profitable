...
19. **Finding engine (task #84) completed — and its own headline is unsafe. READ THIS
   ...
   **To finish properly:** re-run ONLY the kill phase against the ~30 uncovered hypotheses
   (`Workflow({scriptPath: '.../finding-engine-wf_313e6177-d1a.js', resumeFromRunId:
   'wf_313e6177-d1a'})` replays cached agents, so completed work is not repaid), and fix the
   script so a FAILED killer is recorded as `uncovered`, never folded into `survivedClean`.
