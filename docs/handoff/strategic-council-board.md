...
- **Dispatch SHIPPED (`41da6f225`).** Assembled the gate-passing blog HTML from `data/pending-dispatch.json` via the canonical `new-article-skeleton.mjs` + a body renderer:
  ...
  - **`iterate-menu-pricing-experts` workflow — a 6-expert panel (controller, menu consultant, statistician, ag-economist, chef, editor) → judge → build → 3-lens adversarial verify, looping "until the product is perfect."** Founder: "continue in the cadence, iterate until perfect, grounded in expert insights." Each expert reads the piece cold + verifies vs the engine; content-safe picks get built + verified; only survivors applied.
    ...
    - **HELD from round 3 — verification incomplete (session limit), do NOT apply until re-verified:** (i) **own-data ratchet** (economist) — sharp moves are ~10-to-1 upward so relief is drift, a Peltzman-style asymmetry from our own data; (ii) **cohort-size distribution behind the 94%** (statistician) — verifiers flagged a "pack-size artifacts" mischaracterization to fix; (iii) **cooked-pound / "second haircut"** (chef, v9) — a plate-yield layer compounding cooking shrink onto trim tax (cookedYield present for 37 ingredients), the most substantial remaining add. Re-run the panel/build+verify for these after the limit resets (Workflow scriptPath iterate-menu-pricing-experts, resumeFromRunId wf_76643307-207 replays cached agents).
