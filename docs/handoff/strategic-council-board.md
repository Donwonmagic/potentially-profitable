...
12. **Swept the injectors that run in NO chain — their committed output was never
   re-verified.** 10 such `inject-*` scripts exist; 8 reported drift. Classified before
   touching anything, because an out-of-chain injector is as likely to be RETIRED as rotting:
   - **Retired** (sentinel absent from every committed file, must never be run):
     `inject-listen-btn` (0 regions; superseded by `inject-article-listen`),
     `inject-glossary-sitemap` (0 regions, exits 2 — crashing).
   - **DESTRUCTIVE — do not run:** `inject-glossary-explainers`. Its `--check` reports **1**
     stale file while its write path touches **58**, and what it does is **strip `id`
     attributes off `<h2>` headings** that `inject-h2-anchor-ids.mjs` (deploy chain) adds —
     it would destroy 58 ES glossary deep-link anchors. A `--check` that under-reports is
     worse than none: it certifies "up to date" while the write path disagrees.
   - **Genuine rot, fixed + now gated:** `inject-hub-modified-time` (6 hubs stale) and
     `inject-article-author-card` (7 blog posts missing or stale the author card 54 other
     articles carry, incl. `blog/cost-index-2026-07/`). Fabrication gate clean on the bio.
     NB the card injector converges 7→2→0 rather than being a first-run fixed point — the
     same class fixed in `inject-coverage-regime-note` today; stable now, worth fixing later.
   - **Large drift, left alone pending a call:** `inject-feed-discovery` (679 pages),
     `inject-critical-css-nav` (772), `inject-esl-bridge` (65). Too large to apply without
     knowing why they stopped running. **FOUNDER CALL: retire or re-land.**

13. **The adversarial audit caught three real defects in the crosswalk I had just shipped.**
   A 9-agent workflow ground all five code tables and attacked the design. Its findings were
   verified against the committed bytes before acting — and three were correct and serious:
   - **A FABRICATED EDGE, published.** The 5 ENSO rows carry `oni:true` and NO `land`, so
     keying the shared-binding map on `v.land` collapsed them onto one `undefined` key:
     `salmon-fillet` shipped as `code_shared_by_slugs` **with `shrimp`**, under an **empty
     code**. That relationship does not exist — it was an artifact of a missing field. The
     exact class the crosswalk exists to prevent, shipped by the crosswalk.
   - **Two authorities fused as one.** The file's own `_signal_split` says `oni` is the ENSO
     regime gauge, "not a land station" — a NOAA CPC ocean index with no land geography.
     Publishing it as `native_level=weather_region` erased the distinction. Now its own
     authority `noaa_cpc_oni`, code `ONI`, `native_level=enso_oni_index`.
   - **`usda_nass` hard-coded `binding:'exact'`** on all 102 rows while **13 NASS series are
     shared by 39 slugs** — the disclosure built for HS, missing for NASS. Now derived the
     same way. This surfaces something material: **NASS `CATTLE` binds five beef cuts**
     (ground-beef, ribeye, short-rib, striploin, beef-tenderloin), so any per-cut domestic
     production figure is really the whole cattle series. Shared rows 78 → 117.
   My 29-assertion self-test missed all three because no fixture had an ONI-shaped entry or
   a shared NASS series. Both are now regression fixtures; self-test 29 → 42.
   **Audit findings still to triage:** wrong-species HS bindings (`030366` cod), an HS8 code
   absent from the cited substrate, 110 of 226 `sdesc` values not byte-equal to Census SDESC,
   NASS classes that silently don't match (`grapes` TABLE, `green-onion` GREEN), 12 NASS slugs
   with no rows, dead weather stations (`pe-coast` for blueberry), and ERS `scope` being a
   constant that carries zero information.

14. **Wrong-species HS binding corrected — cod's exports were 4.1x overstated.**
   Triaging the audit's highest-severity finding: `030366` was labelled "COD, FROZEN (whole)"
   and bound to `cod`, but the Census substrate the table claims to be verified against calls
   it **"HAKE, FROZEN"**. Cod's import layer was hake's. Removed (cod retains `030251` +
   `030471`, both correct). Downstream, cod's published **US export value corrected from
   $150,312,486 to $36,451,407** — the old number was **4.1x** the truth, and it feeds
   `import_reliance_pct`, the very read ADR-017 §3 nets exports out of.
   Also `080290` was labelled "PECANS" while Census calls it the all-other-nuts residual;
   relabelled with a note that it is never a pecan-only figure.
   **I measured rather than repeating the agent's framing:** it reported "110 of 226 sdesc
   values not byte-equal to Census". Byte-equality is the wrong test — most are legitimate
   paraphrase. Requiring one shared significant word found **exactly 2 genuine contradictions
   out of 147 verifiable codes**, and both were real. Now gated in
   `build-ingredient-codes.mjs`: a code whose label shares no significant term with the
   Census SDESC fails the build. Probe: mislabelling `030251` -> exit 1; restored -> exit 0.

15. **Second audit pass on the crosswalk — two more blockers, both mine, both fixed.**
   The hazard lenses (honesty / licensing / decidability) returned after the ground phase:
   - **`usda_ers` hardcoded `binding:'exact'` on all 71 rows** while 11 ERS commodities bind
     32 slugs (`Beef: Supply and use - carcass weight` binds 5). I fixed this for NASS and
     missed ERS. Now derived. Shared-code disclosure across the file: 78 → 117 → **149**.
   - **28 rows published a supply-share PERCENTAGE** (`stated origin: Mexico 88%`) in a file
     whose own honesty note promises "no price, no volume, no share". A self-contradiction
     inside one artifact. `originNote()` now keeps the country and strips the figure; a
     regression asserts NO published row carries a `%` anywhere.
   - **Registered in `llms.txt`, EN + ES** (ADR-015 mandatory; the audit noted a prior fix
     had landed EN-only, so both were done and verified). NB the generator body is a JS
     template literal — a backtick in the copy terminates it; use quotes.
   - **I disproved one audit claim rather than acting on it:** "granularity carries zero
     information / is a many-to-one function of authority." It genuinely varies within
     `census_hs` and `usda_nass` — 351 of 474 rows. True only for the four small authorities.
   Self-test 42 → **52**.
   **OPEN — FOUNDER CALL, a genuine ADR conflict:** ADR-013 line 75 says "Redistribute the
   raw public-domain series + HS↔ingredient crosswalk **(CC0)**"; ADR-015's later
   `muntin_compilation` split (which the catalog encodes machine-readably) makes it
   **CC-BY**. I shipped CC-BY and did NOT silently flip it — this is a rights decision about
   your company's work. Whichever way it goes, ADR-013 line 75 needs an explicit supersession
   note so the next reader doesn't hit the same fork.

16. **Founder calls answered 2026-07-28 — two implemented, two queued.**
   - **Yield cut-specs → "label the cut spec, keep both."** `build-ingredient-yield-pages.mjs`
     now renders a "What this yield measures" block from `ingredient-depth.json.cutSpec` on
     all 118 EN + ES pages (115 have a spec; the 3 without render nothing rather than
     inventing one). Swiss chard's page now says the 75% and the 92% differ because one counts
     the stems. **No published trim-tax math changed.** ES has no `cutSpec_es`, so the framing
     is Spanish and the source's own wording is quoted `lang="en"` — better than hiding the
     detail from Spanish readers. `check-yield-agreement` waivers rewritten from "unresolved"
     to "resolved by disclosure"; the gate still blocks an 8th.
   - **Crosswalk license → keep CC-BY.** ADR-013 line 75 now carries an explicit
     **SUPERSEDED IN PART** note pointing at ADR-015's split, so the fork can't be
     re-litigated from that line alone. The raw series it names stay CC0.
   - Queued: investigate the 3 large-drift injectors; add a locale to the /open generators.

   **FOUND WHILE DOING THIS — `researchTargets()` IS DEAD CODE.** `scripts/lib/cost-research.mjs`
   emits `/cost-index/menu-pricing/` (+ its study pages and the CC-BY dataset) and its own
   docstring says "Consumed by build-cost-index-pages.mjs" — but **nothing imports it**.
   That page has no live generator; the committed HTML is the only copy, and an edit to the
   engine that renders it silently does nothing. I reverted my own (inert) edit rather than
   leave code that looks live. `check-menu-pricing-render` still passes because it is
   content-scoped to specific regions. **Needs a call: re-wire it into
   `build-cost-index-pages.mjs` (restoring the documented architecture) or declare the page
   hand-maintained and delete the dead export.**

   **Hazard re-confirmed:** running `build-cost-index-pages.mjs` stripped injector regions
   from 469 files AND regenerated `open/index.html` into a shape `inject-open-cards` can no
   longer anchor into. Recovery is: re-run the injector chain, then the post-generation chain
   (lazy-script-loader → dataset-date → speakable → sitemap LAST, since stamping changes
   `dateModified` and therefore `lastmod`), and revert `open/`.

17. **The 3 large-drift injectors: investigated, then all three re-landed.**
   Founder call was "investigate, then report". None was superseded — all three had **partial
   coverage**, which the evidence made unambiguous: `index.html` carried 5 feed-discovery
   `rel="alternate"` links (2 RSS) while `cost-index/apple/` carried 3 (0 RSS).
   - `inject-feed-discovery` (679 pages) — additive `<head>` links; landed, idempotent.
   - `inject-esl-bridge` (65 articles) — resolves its target from the page's own `hreflang`
     alternate and refuses to stamp without one, so it **cannot** mint a dead link now that
     `check-hreflang-orphans` RULE 2 proves every remaining hreflang resolves on disk. Landed.
   - `inject-critical-css-nav` (772 pages) — **this one was genuinely stale, not merely
     unrun.** It hardcoded `#E8E2D6`, the RETIRED `--line` value that
     `migrate-warm-palette.mjs` (deploy chain) exists to purge; landing it as-is re-introduced
     a retired colour on 770 pages and tripped the warm-palette gate. Fixed at source to
     `var(--line)` (the same rule already used `var(--cream)`, proving tokens resolve in that
     context), then purged + re-stamped. **Second defect found in it: its `--check` is
     PRESENCE-only** — it does not detect a changed block, which is exactly how the stale hex
     survived. Recorded in the check-all comment; do not mistake its green for a content gate.
   All three are now in the **wrangler deploy chain** (they are sitewide, not
   cost-index-specific), so a page regeneration re-applies them instead of stripping them —
   the rot that put them here. feed-discovery + esl-bridge also get real `--check` gates.
   Baseline pruned 7 → 6: "CSS cache-bust (idem)" now passes and no longer needs an exemption.

18. **Spanish /open — SCOPE CORRECTION before building (the 4th founder call).**
   The chosen option was "add a locale to the generators — the only version that doesn't
   drift". Correct for the generated explorers, but scoping it first showed **it reaches only
   3 of the 16**:
   - **Generator-backed (3):** `build-open-labor-page.mjs` (23 translatable strings),
     `build-open-demand-page.mjs` (17), `build-open-recalls-page.mjs` (35) — **75 strings
     total**. Each is monolithic with English inline and no locale seam, so each needs a
     locale table extracted; the pattern to copy is `emitSeasonalityHub(locale)` in
     `build-cost-index-pages.mjs`, which is why `/open/seasonality/` is the ONE explorer that
     already has an ES counterpart.
   - **NOT generator-backed (13):** `imports` (948 lines), `exports` (1167), `origins` (1001),
     `energy` (733), plus availability, crop-condition, food-dollar, landings, meat-chain,
     nass-domestic, oni, record, weather. These are **hand-written HTML with no generator at
     all** — there is nothing to add a locale to. Translating them means roughly **13,000
     lines of new Spanish technical prose**, every number of which must clear the fact gate
     and the voice canon.
   **So the call needs a second half:** for the 13 static explorers — (a) convert them to
   generators first, then add the locale (large, but makes them non-drifting like the other
   3), (b) hand-translate as static ES pages (fastest to ship, drifts on every edit), or
   (c) leave them EN-only and declare it, shipping ES for the 3 generated ones.
   **Nothing was built for this yet** — reporting the shape before spending effort on the
   wrong one.

19. **Finding engine (task #84) completed — and its own headline is unsafe. READ THIS
   BEFORE QUOTING IT.** 216 agents, 14.6M subagent tokens, 6.5h. It reports
   `survivedClean: 3`. **That number is an artifact, not a result.**
   **97 of 216 agents ERRORED**, essentially all of them the `kill:*` adversarial verifiers,
   on `You've hit your session limit`. Of the kill verdicts that ACTUALLY RAN:
   **24 `kill`, 35 `survives-weakened`, and ZERO `survives-clean`.** The 3 "clean survivors"
   are hypotheses whose killers never returned — absence of refutation counted as survival.
   Roughly **12 of 42 computed hypotheses got real 4-lens coverage; ~30 got none.**
   Do not publish anything from this run on the strength of its survival labels.
   **What IS worth keeping — convergent validation.** Independently, executing code against
   the real files, the engine landed on two defects I found and shipped the same day:
   - *confirmed*: "the lock/float conformal band's published walk-forward coverage does not
     hold uniformly across the panel" — the coverage-regime finding (ADR-022, `e0b28a3c6`).
   - *partial*: "the two edible-yield reference tables disagree on 46 of 112 shared
     ingredients" — the yield finding resolved by cut-spec disclosure (`11b5f5302`).
   Two independent methods converging on the same two defects is real signal about the
   defects. It is NOT evidence the engine's other 40 hypotheses are sound.
   Raw outcomes across the 42 computed: 8 confirmed, 23 partial, 11 refuted. The refuted
   ones are useful too — e.g. it refuted "the hedge-vs-mirror substitute verdict is merely a
   restatement of shared seasonality."
   **To finish properly:** re-run ONLY the kill phase against the uncovered hypotheses, and
   fix the script so a FAILED killer is recorded as `uncovered`, never folded into
   `survivedClean`.
   **DO NOT trust `resumeFromRunId` to replay cached agents here.** That advice was wrong and
   it cost ~20M tokens across two attempts. The container rolled back three times, destroying
   the run cache; the resume re-ran the entire completed Execute phase from scratch. The tell
   is on disk: a cache hit writes NO new agent transcript, so if fresh transcript files appear
   under today's date, nothing is being reused. Check transcript dates BEFORE resuming, and
   prefer a small standalone job seeded from the committed salvage over resuming a long run.

20. **The salvaged discovery corpus has ZERO survivors — and "confirmed" never meant what it
   looks like.** `scripts/regrade-discovery-findings.mjs` (21 self-test assertions) rebuilds
   the hypothesis↔kill join that `salvage-workflow-findings.mjs` dropped — KILL_SCHEMA objects
   carry `{lensName, verdict, reasoning, fatalProblem, requiredCaveat}` and **no reference to
   the hypothesis they judged**, so the engine's survivor rule
   (`kills.length >= 4 && !kills.some(k => k.verdict === 'kill')`) was uncomputable. The join
   is rebuilt from rare numeric tokens, because reviewers quote a finding's statistics verbatim
   (`BAND 63 / SEASON 84`, `D=21`, `p=0.566543`). All 21 verdicts rejoin with zero ambiguity.
   Output: `docs/handoff/discovery-run/graded-findings.json`.
   - **5 hypotheses got a complete 4-lens panel. All 5 were KILLED. Survivors: none.**
   - **None of the 7 `confirmed` findings has ever met the panel** — 6 have zero lenses, 1 has
     one. `execOutcome: confirmed` is the VERIFIER's self-report on its own execution; `grade`
     is the adversarial result. Conflating them is how an unreviewed claim gets published.
   - **The kills fail the same way three times:** the statistic restates the pipeline's own
     dependency graph rather than measuring the world; the "novel" join is already published
     on muntin.digital under CC-BY; two "independent" tables are bit-identical (66 of 66
     agreements are identical floats — one was copied from the other). **The engine kept
     rediscovering Muntin's own derivations and reporting them as facts about the market.**
   - **Consequence for Deepen:** seeding `survivors.concat(weakened).slice(0, 8)` off exec
     outcomes would build a whole second round on pipeline tautologies and inherit the defect.
     Re-seed instead around the kill pattern — constrain hypotheses to statistics whose inputs
     sit UPSTREAM of Muntin's own processing, require independence to be demonstrated rather
     than assumed, and check novelty against muntin.digital's published surfaces first.
   - **Publication safety:** 29 of 38 `claim` strings still carry unfilled `{N}`/`{M}`/`{P}`
...
21. **The kill panel on the 7 `confirmed` findings completed: 7 tested, 7 KILLED, 0 survivors**
   ...
   **12 findings have now been fully paneled and all 12 died. The discovery run's publishable
   finding count is zero.** Representative kills, all verified independently before acting:
   - The substitute-co-movement finding built its headline on `cavg(a,b) = (cdir(a,b)+cdir(b,a))/2` —
     **the undirected pair score ADR-015 retired of record.** ADR-015's own worked example of the
     asymmetry that must be preserved (chicken-breast→thigh 5/6 is not thigh→breast 5/5) is the
     exact pair it led with, collapsed to a 0.917 that exists in neither direction. Grepped the
     live tree: **nothing shipped computes an undirected average** — only the comment warning
     against it at `scripts/lib/cost-events-analysis.mjs:13`. The engine invented the violation;
     the site is clean. Its "category-matched control" was also 8-of-10 same-species pairs
     (6 Lactuca sativa lettuces, cherry tomato/tomato, broccoli/cauliflower), so it restated
     botany, not a market relationship.
   - The flat-is-dead-price-discovery finding reported p = 0.00000 on 10 series of which **5 are
     one AMS Boston herb table** (identical start, identical n=130, synchronized change weeks) —
     effective n ≈ 6 blocks, exchangeability violated — and its label is a near-deterministic
     function of its own outcome. Also factually false for 3 of 10 (kale moves 17/51).
   - **TWO KILLS WERE ABOUT OUR CODE, NOT THE FINDING, AND BOTH WERE REAL** — fixed in `22e517ebe`.
     See the `fetch-food-recalls` entry below. This is where the run's actual value landed:
     **the panel was a better auditor than the generator was a discoverer.**

22. **`fetch-food-recalls.mjs` shipped a silent zero for six years of jalapeño recalls.**
   `ingredientKeywords()` lowercased the display name then stripped `[^a-z ]` to spaces, so
   "Jalapeño" → keyword `"jalape o"`, which matches no product text ever. 4 openFDA rows name
   jalapeños — one is literally `"Supreme Jalapenos Diced"` — and none carried the slug. **A
   published zero that was a string bug, reading as a food-safety fact.** Fixed by folding
   accents (NFD + strip combining marks) BEFORE the `[^a-z ]` strip on **both** sides of the
   matcher; an accented product description otherwise fails to match an un-accented keyword.
   1 of 169 tracked names was affected. New `--retag` re-runs matching over stored rows offline
   — **deliberately partial**, since rows matching nothing at fetch time were never written and
   only `--live` recovers those.
   **Related disclosure gap, also fixed:** openFDA covers FDA-regulated food only. Meat, poultry
   and egg products are USDA/FSIS and absent from the endpoint — verified: beef, pork and every
   chicken slug have **zero** product-text mentions across all 718 rows, `product_type` uniformly
   `"Food"`. A zero there is guaranteed by the source, not measured. Now shipped as a
   `coverageLimits` block including an explicit **"recall counts are not comparable across
   slugs"** warning. Never rank ingredients by recall count.
