...
- **STILL OPEN:** **NOAA Fisheries** (seafood domestic pair) — crosswalk staged (`ingredient-noaa-codes.json`), but the FOSS API was returning **503 (their server down)**; retry later, then ground it (probe first). **The presentation/harmony render** (the CHAIN + harmony reads on the `/cost-index/menu-pricing/` island) is still unbuilt — all this fused data isn't yet visible to a reader. Then the **research paper** resumes.
