...
**Still ahead on the CHAIN:** citable per-ingredient "Supply picture" block + schema.org (surface 2, AEO/SEO); publish the 13 open-data explorers (surface 3); optional co-mover "shared timing, not cause" chip caveat; the audit's remaining LOWs (per-capita group-scope label; the 2%-floor "trace" style is now moot — bars removed).
