...
- **Dispatch SHIPPED (`41da6f225`).** Assembled the gate-passing blog HTML from `data/pending-dispatch.json` via the canonical `new-article-skeleton.mjs` + a body renderer:
  ...
  - **`iterate-menu-pricing-experts` workflow — a 6-expert panel (controller, menu consultant, statistician, ag-economist, chef, editor) → judge → build → 3-lens adversarial verify, looping "until the product is perfect."** Founder: "continue in the cadence, iterate until perfect, grounded in expert insights." Each expert reads the piece cold + verifies vs the engine; content-safe picks get built + verified; only survivors applied.
    ...
    - **✅ ROUND 3 PANEL COMPLETE — all builds shipped, none held.** The `wf_76643307` session-limit casualties were all recovered by hand-verification against the engine: 3b (cooked-pound/second-haircut), 3c (own-data ratchet/rockets-and-feathers), 3d (cohort-depth). The dispatch now carries, end to end: meat-buying-season lead + freeze-play $ callout → posture split + named floats + worked card + two-clocks + band-cost callout → trim tax + cut-by-cut table + **served-pound (fire) layer** + posture-hazard caution → swap-mirror with **co-movement depth** → don't-reprint 77-day + **rockets-and-feathers asymmetry**. Returns are narrowing hard (each round now deepens an existing section rather than adding a pillar). **Next genuine lever = the invoice-personalization instrument** (Approach 1, teed up at line 37) — a build, not a prose pass; awaits founder go. Optional: mirror the served-pound + ratchet + cohort-depth findings into the field-report study (`cost-index/menu-pricing/study/` + `data/cost-research-study.json`) for paper/dispatch parity.
