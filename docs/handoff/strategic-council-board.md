...
- **#2 Ship the real multi-year history** — methodology boasts a multi-year backtest; public `series.json` is ~5 weeks. The deep backfill (`data/cost-index-history.json`, used only internally for the conformal backtest) should feed public `series.*` + extend Dataset `temporalCoverage`; LABEL backfilled points reconstructed. Resolves the credibility mismatch. (journalist #1, econ #2)
- **#3 Basket transparency** — `/cost-index/basket/index.html` EXISTS but is **linked from nowhere** (grep=0); link it from the hub composite drawer, publish the weights + the baseline-window dates, and fix `+3.2%` false precision (agreement 0.523 ≈ coin-flip → coarser band or pair with dispersion). Add the reconcile CI assertion (word=sign(pct)). Also fix Dataset `datePublished`==`asOf` (erases history).
- **#4 Hub "all readings" comparison table** — `aggregateRows()` already computes every cell (today only a CSV); render a sortable price-FREE table (ingredient · direction · verdict · as-of · mini indexed-spark) — the "terminal" scan view. Vanilla JS sort, degrades gracefully.
- **#5 "What changed since last week"** — `data/cost-index-readings.prev.json` EXISTS (49KB) and is discarded; "N moved into Watch since last week" is the best honest return-loop for a static product. Surface the hidden `cost-index/weekly/` archive + `/feed.xml` near the signup.
- **#6 Portion-cost bridge** ($/unit → $/plated portion) — `data/ingredient-yields.json` + `build-ingredient-yield-pages.mjs` EXIST; operator's #1. AUDITOR RED LINE: keep user-input/illustrative, NEVER a narrated plate-dollar margin (that's the May-2026 fabrication shape).
...
- **P2**: window/muntin motif on interior pages + mobile (port `muntinField()` SVG); earn the golden-hour warm layer in 2-3 live moments; kill `#C5A059` palette drift in `site-article.css` viz-* warn states; promote inline styles → tokens (fix `#14161A`≠`--ink`); dark-mode hero gradient parity; SearchAction JSON-LD; `/status/` real timestamps; OG AVIF/WebP + flip format gate; honest "proof you can check" cluster on /trust/ or /receipts/; ES audio backfill (warn-only).
