...
**Remaining standing-queue item (one genuine fork — founder call):** **`tools/start` prune.** It exists EN+ES with ~6 inbound links; top-level `start/` is already retired+301. Three honest options: (a) **retire+301** like the tools migration (cleanest removal, but needs the Worker redirect map + repoint the 6 links — `_redirects` is at CF's 100-rule cap); (b) **noindex-shelf** (the `method/` treatment — drops it from search/sitemap, keeps it live, no redirect/link work, reversible); (c) **keep it** — it's a tool-discovery page, arguably on-funnel, not clearly worth pruning. Not executed: it's a genuine approach-fork the board itself flags as entangled, surfaced rather than guessed.
