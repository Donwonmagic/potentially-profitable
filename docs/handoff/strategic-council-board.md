...
**REMAINING — P1d (Cost Index data-product) + P2 (polish), not yet done (resume here):**
- **P1d — Cost Index as a data product** (the audit's strongest "make the flagship look like a flagship"): in `build-cost-index-pages.mjs` (calendar-sensitive — commit SOURCE, refresh workflow regenerates). Turn the hub index from a name+verdict-pill list into a sortable rich table (indexed value + mini-sparkline + trend% + verdict, sortable by movement, reuse `.mtn-spark`); add a hero KPI band (tracked count · last refresh · # moving this week) + a composite movement index line (indexed to 100, relative — NO live cents on hub); enrich sparklines with the 80% confidence band (already computed) + endpoint date labels + min/max dots; surface the hidden `cost-index/weekly/` archive ("Past weekly reads →") near the email signup; reconcile the hub "refreshed" date vs ingredient "as of" labels.
