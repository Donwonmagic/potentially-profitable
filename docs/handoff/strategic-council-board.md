...
- **REMAINING (rendering, from the spine — single source of truth, gate every number+citation before it ships):** (1) engine join `ingredient-depth.json` → depth on playbook cards + surface all 134 (incl unpriced partial cards) + extend `check-cost-research` to ground depth numbers; (2) the ~94 ingredient pages get the per-item profile; (3) the paper-formatted **research study** wrapping the tool (abstract, cited sections, kitchen translations, contribution, methods, limitations, references, ScholarlyArticle JSON-LD — stands on peer-reviewed work, never claims to BE peer-reviewed); (4) glossary vocabulary (WHC, thaw purge, ethylene, gelatinization, AP/EP, menu costs, cross-price elasticity) + a FEW deep library articles; (5) tools consume enriched yields; open-data publishes the new sets; autolink cross-linking. New staples need `apCents:null` handling + lamb `cat:beef`→lamb fix. Tasks #37-39, #42-45.
