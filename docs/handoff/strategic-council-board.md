...
**IN FLIGHT / NEXT:** Workstream F — **educated-inference research pages** (repackaging, not aggregating): what-moves-together clusters, trim-tax reality, steady-vs-wild volatility, how-long-shocks-last, cheapest-month calendar, invoice-vs-reference method. Then Phase 5 finish: CTA-canon entries, final full adversarial audit. **DOI (Zenodo)** minting is operator-side (on-site `DataCatalog`+CC-BY+cite are in place). **Queued:** the complete site reinvigoration (build surfaces to fit it; house tokens only).
