...
- **STILL OPEN:** **NOAA Fisheries** (seafood domestic pair) — crosswalk staged (`ingredient-noaa-codes.json`), but the FOSS API was returning **503 (their server down)**; retry later, then ground it (probe first). **Weather/ONI pressure integration** (fuse the held observations into a read, with a double-count guard) still ahead. **The dataset-explorers workflow** needs its outputs published as Artifacts + audit grades once it completes (resume for crop-condition). **The presentation/harmony render** (the CHAIN + harmony reads on the `/cost-index/menu-pricing/` island) is still unbuilt — all this fused data isn't yet visible to a reader. Then the **research paper** resumes.
