...
1. **Freshness-clock fix** (`tools/_shared/cost-pressure.js:96`) — CHOSEN, in progress. Measure staleness against a passed-in `now` (keep `assess()` pure — add `opts.now`, default to `asOf` for back-compat so the gate's recompute stays deterministic; build passes build-date, client passes `Date.now()`). Let `weeksPerNotch` decay + 8-wk `under_review` floor fire on real age. Add a `check-pressure-freshness` assertion that freshness reflects real age. Verify: the current 2026-06-08 snapshot should trip `under_review` at today's date.
