...
| **Invoices + Recipes** | E1 vendor-vs-market (wired end-to-end), E2 per-recipe + digest, recost hero loop, `/cost`, **✅ E4 silent-bleed, E5 blast-radius, E6 margin-map, E14 stress-test (`/stress-test`), E15 cost-history (`/:id/cost-history`) — all routed** | E9 learned-yield (V2) | E9 (needs learned yields persisted) |
| **Invoices + Inventory** | days-of-cover, reorder_due, inventory cost-index valuation; **✅ E7 buy-now-vs-wait routed** (`GET /v1/insights/buy-or-ride` — recent hikes × days-of-cover × market trend, reusing the `reorder` coverage assembly) | E8 theoretical-vs-actual (needs POS, V3) | (tier now actionable; E8 is the crown) |
...
6. **✅ SHIPPED — E7 buy-now-or-ride-it-out (the Invoices+Inventory tier).** `GET /v1/insights/buy-or-ride` — recent `price_hike` verdicts (`verdicts-store.recentPriceHikes`, JOINed to the canonical's inventory catalog item) × that item's days-of-cover (reusing the existing `reorder` assembly) × the market trend → "you've got N days, you don't have to buy at the high" vs "you're nearly out, here's the move." The core needs no recipes — it serves the invoices+inventory operator on its own. **Honesty boundary:** the single-point vendored snapshot yields a trend-based verdict (`trendToVerdict`), never a fabricated peak-retrace. Node-validated (39 checks).
7. **Activate cohort (E10/E11) post-counsel** *(the universal invoice layer)*. Peer percentile is the one insight every invoice operator gets regardless of any other tool — the latent floor-tier upgrade once the antitrust gate clears.

**Sequencing:** 1 + 2 close the floor-tier gaps (most operators, least dependency). 3 unlocks the recipes-only entry. 4 makes the whole ladder legible (and de-risks every "empty" payload). 5 completes the Invoices+Recipes deepening. 6 lights up the Invoices+Inventory tier. 7 waits on counsel. The only Bucket-A/early-Bucket-B gap left is **E8** (theoretical-vs-actual — needs POS sales-mix, V3).
