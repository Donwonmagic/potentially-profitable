...
- **Privacy:** per-org. **Phase:** V2. **✅ decision brain built** — `tools/_shared/buy-or-ride.js` (10 vectors) + the Ledger parity port `buy-or-ride.ts` + `buy-or-ride-suggest.ts` (the honest `trendToVerdict` + `buildBuyOrRideFromContext`), node-validated. The canonical↔inventory bridge is confirmed (`product_canonical.catalog_item_id` → the existing `reorder` coverage assembly); the read-path route (the inventory join) is the next wire.
