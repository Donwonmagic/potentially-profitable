...
- **Spike-vs-structural is honest today; seasonality is NOT.** `tools/_shared/cost-spike.js classify()` (≥8 wk history) → `structural | spike | emerging | easing | flat | insufficient` with `actionBias` re-price/hold/watch; `tools/_shared/cost-verdict.js verdict()` downgrades a `structural` call to **watch** on thin data. But only 41/101 ingredients have ≥6 established months (`data/seasonality.json`), and the index went live 2026-06-05 — so **2-year seasonal claims are not possible until ~mid-2027.** "Hold, it'll revert" must use spike-classifier language ("already pulled back from its peak"), never seasonal language ("tomatoes always spike in February").
...
- **Inventory tables EXIST in the Ledger repo** (migrations 0046–0050: counts, period snapshots, pars with `lead_time_days`, `inventory_coverage` days-of-cover, `cadence-compute.ts` daily buy-rate). "Inventory" is therefore a **real** dataset — used here only as **read-only joins**, never as a system Plate operates.
...
2. **`decomposeMove({ownDeltaPct, marketDeltaPct, confounders})`** — the discrimination function. Screens four confounders (pack flip via `cents_per_base`/`base_uom`; grade/spec via `canonical_id`; promo via period-median + robust-Z; window via the same calendar window), then attributes the unexplained gap → `{attribution:'vendor'|'market'|'mixed'|'inconclusive', marketPoints, vendorPoints, confidence}`. **Gate:** emit a spread only when index `confidence ≥ medium` AND `agreement ≥ 0.66` AND ≥3 same-unit vendor periods exist; otherwise show each side alone, no spread. Spread-of-**changes** only — never spread-of-levels.
3. **`vendorAsk(canonical, decomposition, locale)`** — a thin EN/ES copy layer: the show-your-work vendor script (E1) and the cheaper-vendor recommendation (E2).
...
- **Inputs:** invoices (hike) × inventory (`days_of_cover`) × index (spike-vs-structural).
...
- **Confidence/honesty:** `days_of_cover` is exact; the "may not stick" claim uses `cost-spike.classify` (retrace-from-peak, ≥8 wk) — **never** seasonality until ≥2-yr history (~mid-2027). On thin data `cost-verdict` downgrades to "watch."
- **Scope guardrail:** READS the existing `inventory_coverage`; Plate never asks you to count inventory just for this. Ships only for orgs already on the inventory track.
- **Privacy:** per-org. **Phase:** V2.
...
- **E14 Menu-resilience stress-test** — `stressTestRecipe` × recipes → "If beef jumps 20% like last spring, your ribeye holds at 26% but your short-rib special goes underwater." Labeled **a what-if, not a forecast.** V2.
- **E15 Per-dish cost-history × index** — `recipe_cost_snapshots` history → "Your Caesar's cost is up 9% over 6 months, tracking the market." V2.
...
8. **Seasonality "just hold" claims without ≥2-yr history.** Until then: "not enough history to call this seasonal — treating as structural." Spike-classifier retrace language is fine; seasonal language is not.
...
2. **Honesty audit.** No insight (a) compares levels across bases, (b) asserts seasonality without ≥2-yr history, (c) predicts a future price, (d) presents theoretical as actual, or (e) shows cross-operator levels without the counsel gate. The DO-NOT-BUILD list mirrors these one-to-one.
