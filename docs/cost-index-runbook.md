...
Put the keys in a local `.env` (gitignored — never committed):

```bash
cp .env.example .env        # then edit .env and paste the keys
```
...
Load the keys:
```bash
# for node scripts (Node >= 20.6 — you're on 25):
node --env-file=.env scripts/verify-cost-index-sources.mjs
# for curl / interactive shell (also fixes the "User is not found" empty-key gotcha):
set -a; . ./.env; set +a
```
> Gotcha: `AMS_KEY=… curl -u "$AMS_KEY:"` sends an EMPTY key — bash expands
> `$AMS_KEY` before the inline assignment applies. Use `set -a; . ./.env; set +a`
> (or `export AMS_KEY=…`) first, then curl.
...
```bash
node --env-file=.env scripts/verify-cost-index-sources.mjs --flip
```
...
```bash
node --env-file=.env scripts/verify-cost-index-sources.mjs --discover beef
node --env-file=.env scripts/verify-cost-index-sources.mjs --discover "boxed"
node --env-file=.env scripts/verify-cost-index-sources.mjs --discover --all > /tmp/ams-reports.txt   # full dump to grep
```
...
```bash
node --env-file=.env scripts/verify-cost-index-sources.mjs --flip          # set verified:true
node --env-file=.env scripts/fetch-cost-index-sources.mjs --live --out /tmp/ci.json   # fetch real points
