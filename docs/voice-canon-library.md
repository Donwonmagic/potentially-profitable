...
- **`viz-diverge`** — SIGNED data on a zero-centred axis: a quantity that can go up OR down (a read vs baseline, a basket contribution). Length is magnitude, SIDE is direction — building-cost/positive grows right (rust), easing/negative grows left (teal), with a centred zero line and a `viz-diverge__axis` legend. Direction is read from position + colour + the signed number, never colour alone. Use this, never a one-directional `viz-bars`, whenever the values carry a sign. Rows sort signed-descending so pushers group above easers.
- **`viz-split`** — one stacked bar showing how a WHOLE divides into named parts (e.g. a panel's ingredients as below/flat/above). Segment widths are shares of the whole; counts live in the legend in ink so the bar stays legible in both themes. The honest replacement for a ring that draws only one slice.
- **`viz-gauge`** — a single SIGNED headline value placed as a POSITION on a zero-centred axis (e.g. a basket reading −5% sits left of baseline). Use when one number needs spatial meaning against a neutral midpoint — never map a signed value onto a 0–100 progress ring.
- **`viz-ring`** — composition or share-of-whole at a single point, expressed as a radial fill of 0–100%. Only for a genuine proportion; never for a signed change (use `viz-gauge`) and never when the parts it names aren't all drawn (use `viz-split`).
